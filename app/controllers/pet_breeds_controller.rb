class PetBreedsController < ApplicationController
  ...
  def import
    PetBreed.import(params[:file])
    redirect_to pet_breeds_url, notice: 'Pet Breeds imported.'
  end
  ...
  def destroy
    @pet_breed.destroy
    respond_to do |format|
      format.html { redirect_to pet_breeds_url, notice: 'Pet breed was successfully deleted.' }
  ...
  private
    ...
    def pet_breed_params
      params.require(:pet_breed).permit(:pet_id, :breed_id)
    end
end
